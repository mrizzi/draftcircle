...
_TAG_TEXT = "text_delta"
_TAG_TOOL = "tool_use"
_TAG_DONE = "done"

StreamTuple = (
    tuple[Literal["text_delta"], str]
    | tuple[Literal["tool_use"], ToolUseBlock]
    | tuple[Literal["done"], str | None]
)
...
class AIOrchestrator:
    ...
    async def _stream_query(
        self, prompt: str, opts: ClaudeAgentOptions
        ...
        async for message in query(prompt=prompt, options=opts):
            if isinstance(message, StreamEvent):
                ...
                if (
                    ...
                    yield (_TAG_TEXT, event["delta"]["text"])
            elif isinstance(message, AssistantMessage):
                for block in message.content:
                    if isinstance(block, ToolUseBlock):
                        yield (_TAG_TOOL, block)
            elif isinstance(message, ResultMessage):
                session_id = message.session_id
        yield (_TAG_DONE, session_id)
